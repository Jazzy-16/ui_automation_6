...
public class _03_ProjectTest extends Base {
    ...
    public void validateTheDefaultBookYourTripForm() {
        ...
        Assert.assertTrue(project3Page.returnDatePickerDisabled.isEnabled());
    ...
    public void validateTheBookTripWithRound() {
        //I think I should be creating list of lists, tried but did not work so doing it in a long way..
    ...
    public void validateTheBookingForOnePassenger() {
        ...
        Assert.assertTrue(project3Page.depart_from_to.isDisplayed());

        String[] expectedText = {"Number of Passengers: 1", "Passenger 1: Senior (65+)", "Cabin class: Business"};
        for (int i = 0; i < project3Page.departMessageTag.size(); i++) {
            Assert.assertTrue(project3Page.departMessageTag.get(i).isDisplayed());
            Assert.assertEquals(project3Page.departMessageTag.get(i).getText(), expectedText[i]);
        }
    ...
    public void validateTheOnePassengerAndRoundTrip() {
        ...
        Assert.assertTrue(project3Page.depart_from_to.isDisplayed());
        Assert.assertEquals(project3Page.depart_from_to.getText(), "CA to IL");
        Assert.assertTrue(project3Page.returnMessage.isDisplayed());
        Assert.assertEquals(project3Page.returnMessage.getText(), "RETURN");
        Assert.assertTrue(project3Page.return_from_to.isDisplayed());
        Assert.assertEquals(project3Page.return_from_to.getText(), "IL to CA");
    ...
    public void validateTheTwoPassengerAndOneWay() {
        ...
        Assert.assertEquals(project3Page.departMessage.getText(), "DEPART");
        Assert.assertTrue(project3Page.depart_from_to.isDisplayed());
        Assert.assertEquals(project3Page.depart_from_to.getText(), "NY to TX");
